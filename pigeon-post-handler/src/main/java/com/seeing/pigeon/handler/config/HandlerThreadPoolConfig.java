package com.seeing.pigeon.handler.config;
...
import com.seeing.pigeon.common.constant.ThreadPoolConstant;
...
public class HandlerThreadPoolConfig {
    ...
    private static final String PRE_FIX = "pigeon.";
    /**
     * 业务：处理某个渠道的某种类型消息的线程池
     * 配置：不丢弃消息，核心线程数不会随着keepAliveTime而减少(不会被回收)
     * 动态线程池且被Spring管理：true
     * 你可以根据不同的 groupId 创建不同的线程池实例，每个实例都有一个唯一的名称
     *
     * @return
     */
    public static DtpExecutor getExecutor(String groupId) { //DtpExecutor 动态线程池实例
        return ThreadPoolBuilder.newBuilder()
                .threadPoolName(PRE_FIX + groupId) //设置线程池的名称
