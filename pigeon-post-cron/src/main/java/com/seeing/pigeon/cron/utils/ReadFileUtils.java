package com.seeing.pigeon.cron.utils;
    ...
    public static void getCsvRow(String path, CsvRowHandler csvRowHandler) {
        try {
            ...
            CsvReader reader = CsvUtil.getReader(new FileReader(path),
                    new CsvReadConfig().setContainsHeader(true)); //表示 CSV 文件的首行为标题行
            reader.read(csvRowHandler); //定义了如何处理每一行数据，具体看TaskHandlerImpl
        } catch (Exception e) {
