package com.seeing.pigeon.stream.sink;
...
import com.seeing.pigeon.common.domain.SimpleAnchorInfo;
import com.seeing.pigeon.stream.utils.LettuceRedisUtils;
 ...
 */
@Slf4j
public class PigeonSink implements SinkFunction<AnchorInfo> {
    ...
    private void realTimeData(AnchorInfo info) {
        try {
            LettuceRedisUtils.pipeline(redisAsyncCommands -> { //传入一个回调函数
                List<RedisFuture<?>> redisFutures = new ArrayList<>();

                /**
                 * 0.构建messageId维度的链路信息 数据结构list:{key,list}
                 * key:Pigeon:MessageId:{messageId},listValue:[{timestamp,state,businessId},{timestamp,state,businessId}]
                 * 使用 LPUSH 命令将链路信息存储到 Redis 的 List 中，并设置过期时间为 3 天
                 */
                String redisMessageKey = StrUtil.join(StrUtil.COLON, PigeonConstant.CACHE_KEY_PREFIX, PigeonConstant.MESSAGE_ID, info.getMessageId());
                 ...
                 * 使用 LPUSH 命令将链路信息存储到 Redis 的 List 中，并设置过期时间为当天
                 */
                 ...
                 * 使用 HINCRBY 命令将链路信息存储到 Redis 的 Hash 中，记录每个状态的计数
                 */
            ...
            log.error("PigeonSink#invoke error: {}", Throwables.getStackTraceAsString(e));
        }
    }
}
